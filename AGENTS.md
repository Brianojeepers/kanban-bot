...
- `rate_limit.py`: in-memory rolling-window limits returning 429 with `Retry-After`: 10 failed sign-ins per minute per client address, and 10 chat messages per minute per user. They reset when the app restarts.
